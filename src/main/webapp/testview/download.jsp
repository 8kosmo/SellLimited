...
/* 	out.print("bs_file: 8->euc"+bs_file);		
	out.print("<br>");		 */
	String filePath = "\\\\192.168.0.187\\itemPhoto"; // 절대경로.	
	//가져온 파일이름을 객체화 시켜줌. - 파일이 있는 물리적인 경로가 필요함.
	File file = new File(filePath,photo_name.trim());
	//mimeType이 브라우저가 인식할 수 없는 값이면 브라우저는 무조건 다운로드 처리하도록 설계되어 있음.
	//타임타입이 부재인 경우 브라우저가 모르는 마임타입으로 변경하여 줌.
	...
	if(request.getHeader("user-agent").indexOf("MSIE")==-1){
		downName = new String(photo_name.getBytes("UTF-8"),"8859_1");
	}else{
		downName = new String(photo_name.getBytes("EUC-KR"),"8859_1");
	}
	//응답헤더에 한글처리한 이름 정보를 담아줌.
   	response.setHeader("Content-Disposition", "attachment;filename="+downName);
  	//해당 파일을 읽어오는 객체 생성해 줌. - 이때 파일명을 객체화 한 주소번지가 필요함.
