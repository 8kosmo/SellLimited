...
function sellerReg(shop_id , wish_id , memo){
	...
	jPrompt(shop_id+' 판매자 : '+wish_id, '메모입력', '재팬스타일', function(r) {
		if( r ){
			...
			$.ajax({
				...
				success: function(d,s) {
					if(d.result == '_ok') {
						alert(d.message+'님 관심판매자가 등록 되었습니다.\r\n우측 관심판매자에서 등록하신 판매자 정보를 확인하실수 있습니다.');
					}else{
...
// 관심상품 등록
function recomReg(shop_id , img_url , link_url , goods_name , goods_price){
	...
	$.ajax({
		...
		dataType: 'json',
		data: { shop_id : shop_id , img_url : img_url , link_url : link_url , goods_name : goods_name , goods_price : goods_price  , user_id : user_id},
		success: function(d,s) {
			if(d.result == '_ok') {
				alert('관심상품이 등록 되었습니다.\r\n우측 관심상품에서 등록하신 상품을 확인하실수 있습니다.');
			}else{
...
function option_slide(){
	...
	if($("#option_slide").css("display") == "none"){
		...
		$("#opslide_img").attr("src" , "/M/images/btn_option_off.png");
	}else{
		$("#option_slide").slideUp("slow");
		$("#opslide_img").attr("src" , "/M/images/btn_option_on.png");
	}
}
// 모바일 마이페이지 메뉴 슬라이드
